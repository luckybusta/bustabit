...
$(function(){
    ...
    $('#autoBet_button').click(function(){
        if(!user.connected){
            window.location.href = "https://www.moneypot.com/oauth/authorize?app_id=2906&response_type=token&state=Meh&redirect_uri=https://luckybusta.github.io/bustabit/. ";
            return;
        }
        if(AutoRunning){
            AutoRunning = false;
...
function placeBet(stopAt, bet, currentAt, stake, callback){
    ...
    getHash(function(){
        var odds = 1000000.999999/currentAt;
        var rangeWin = Math.floor(Math.pow(20000,320000)*(odds));
        $.ajax({
            type: "POST",
